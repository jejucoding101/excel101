...
> ⚠️ **막힐 수 있는 포인트**:
> - **SUMIF 결과가 0으로 나와요** → 조건 텍스트를 정확히 입력했는지 확인하세요. `"납부"` ≠ `"납부 "` (뒤에 공백이 있으면 다른 값입니다)
> - **SUMIFS와 SUMIF의 차이가 헷갈려요** → SUMIF는 조건 1개, SUMIFS는 조건 여러 개입니다. 인수 순서도 다릅니다: SUMIF(범위, 조건, 합계범위) vs SUMIFS(합계범위, 조건범위1, 조건1, ...)
> - **음수 금액(-15000)이 합계에서 빼지는 게 맞나요?** → 네! 지출은 음수로 입력했으므로 SUM이 자동으로 빼줍니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **SUMIFS의 인수 순서가 헷갈려요** → SUMIFS의 첫 번째 인수는 **합계 범위**이고, 그 뒤에 (조건범위, 조건) 쌍이 반복됩니다. SUMIF와 순서가 다르니 주의하세요!
> - **조건에 셀 참조를 쓸 수 있나요?** → 네! `=SUMIF(B1:B6, "서울", C1:C6)` 대신 `=SUMIF(B1:B6, E1, C1:C6)`처럼 조건을 셀로 참조할 수 있습니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **COUNTIF 범위를 행 방향으로 지정하는 법을 모르겠어요** → `B1:F1`처럼 가로 범위도 사용 가능합니다. 범위 방향에 제약은 없습니다
> - **출석/결석/지각의 글자가 정확히 일치해야 하나요?** → 네! "출석"과 "출 석"은 다른 값입니다. 데이터 입력 시 일관성이 중요합니다
