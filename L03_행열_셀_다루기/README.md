...
![시간표 완성 결과 — 병합 제목, 요일, 교시별 과목이 채워진 시간표](images/practice_result.png)
...
**📋 완성 결과** (아래 작업을 모두 수행한 후):

| | A | B | C | D(새로 삽입!) | E | ~~F(숨김)~~ |
|--|---|---|---|---|---|---|
| 1 | 이름 | 나이 | 성별 | **전화번호** | 혈액형 | ~~거주지~~ |
| 2 | 김OO | 25 | 남 |  | A | ~~서울~~ |
| ~~3~~ | ~~삭제됨~~ | | | | | |
| 3 | 박OO | 28 | 남 |  | O | ~~대구~~ |
| ... | ... | ... | ... | ... | ... | ... |

그리고 아래 작업을 순서대로 해보세요:
1. **C열(성별) 뒤에 "전화번호" 열 삽입** → D열 머리글 우클릭 → 삽입
2. **3행(두 번째 사람) 삭제** → 3행 번호 우클릭 → 삭제
...
| | A(자동맞춤) | B(너비 15) | C(너비 15) |
|--|---|---|---|
| 1 | 가 | 좋은 아침 | Hello |
| 2 | 가나다라마바사아자차카타파하 | 점심 맛있게 | Good morning |
| 3 | 안녕하세요 | 저녁 | Hi |

> 💡 A열은 더블클릭으로 자동 맞춤 → 가장 긴 텍스트에 맞춰짐. B~C열은 "열 너비 = 15"로 동일하게!

1. A1에 "가", A2에 "가나다라마바사아자차카타파하" 처럼 다양한 길이의 텍스트 입력
2. A열 머리글 경계선을 **더블클릭** → 가장 긴 텍스트에 맞춰 자동 조절!
3. A~C열 전체를 선택 후, 우클릭 → "열 너비" → `15` 입력 → 모두 동일하게
