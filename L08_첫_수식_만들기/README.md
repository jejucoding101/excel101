...
> ⚠️ **막힐 수 있는 포인트**:
> - **수식을 입력했는데 텍스트로 표시돼요 (=B2*C2가 그대로 보임)** → 셀의 표시 형식이 "텍스트"로 되어있을 수 있습니다. `Ctrl+1` → "일반"으로 변경 후 다시 입력하세요
> - **`Alt+=`를 눌렀는데 반응이 없어요** → 합계를 구할 숫자가 바로 위에 있어야 합니다. D7 셀(합계 위치)이 D2:D6 바로 아래에 있는지 확인하세요
> - **수식을 복사했는데 결과가 다 0이에요** → 채우기 핸들로 복사했는지 확인하세요. 복사-붙여넣기(Ctrl+C/V)와 채우기 핸들 복사는 다를 수 있습니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **수식을 입력했는데 결과가 예상과 달라요** → 연산 우선순위를 다시 확인하세요: `^` → `*`, `/` → `+`, `-` 순서입니다
> - **A열에 수식을 입력했는데 "B열(결과)"에 뭘 써야 하나요?** → A열에 수식을 입력하면 자동으로 결과가 표시됩니다. B열에는 직접 관찰한 답을 적어보세요
...
> ⚠️ **막힐 수 있는 포인트**:
> - **`Ctrl+~`를 눌러도 반응이 없어요** → 키보드에 따라 `` ` `` (백틱) 키의 위치가 다릅니다. 보통 `1` 키 왼쪽, `Tab` 위에 있습니다. `Ctrl`과 함께 누르세요
> - **수식 표시 모드에서 열이 넓어졌어요** → 정상입니다! 수식 텍스트가 길어서 열이 자동으로 넓어집니다. 다시 `Ctrl+~`를 누르면 원래대로 돌아옵니다
