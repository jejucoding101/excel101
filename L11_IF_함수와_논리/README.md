...
> ⚠️ **막힐 수 있는 포인트**:
> - **IF 수식이 너무 길어서 어디가 잘못됐는지 모르겠어요** → 괄호 개수를 세어보세요. `IF(`가 4개면 닫는 `)` 도 4개여야 합니다. 수식 입력줄에서 괄호를 클릭하면 대응하는 괄호가 강조됩니다
> - **"수" 대신 숫자 결과가 나와요** → `"수"`처럼 반드시 **큰따옴표**로 감싸야 텍스트로 인식됩니다. 따옴표 없이 `수`만 쓰면 오류가 발생합니다
> - **조건부 서식에서 "불합격" 텍스트를 찾으려면?** → 6강에서 배운 "텍스트 포함" 규칙을 사용하세요: 조건부 서식 → 셀 강조 규칙 → 텍스트 포함 → "불합격"
...
> ⚠️ **막힐 수 있는 포인트**:
> - **AND/OR 함수를 IF 안에 넣는 위치를 모르겠어요** → `=IF(조건, 참값, 거짓값)`에서 "조건" 자리에 AND/OR을 넣으면 됩니다: `=IF(AND(B2>=90,C2>=90), "장학금", "해당없음")`
> - **AND와 OR의 차이가 헷갈려요** → AND는 **모든 조건이 참**, OR은 **하나라도 참**이면 됩니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **IFERROR를 어디에 감싸야 하나요?** → 오류가 날 수 있는 수식 전체를 감쌉니다: `=IFERROR(D2/B2, "-")`. IFERROR의 첫 번째 인수가 원래 수식, 두 번째가 오류 시 표시할 값입니다
> - **0으로 나누는 오류가 뭔가요?** → 수학에서 0으로 나누기는 불가능합니다. 엑셀은 `#DIV/0!` 오류를 표시합니다
