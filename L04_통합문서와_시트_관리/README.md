...
### 실습 1: 이력서 시트 분리하기 (10분)

**목표**: 항목별로 시트를 나누어 체계적인 이력서를 만듭니다.
...
> ⚠️ **막힐 수 있는 포인트**:
> - **시트 이름을 바꿀 수 없어요** → 시트 탭을 **더블클릭**하면 이름이 편집 모드로 바뀝니다. 한 번 클릭은 시트 이동입니다
> - **시트 이름에 특수문자를 넣었더니 오류가 나요** → 시트 이름에는 `\ / ? * [ ]` 등의 특수문자를 쓸 수 없습니다
> - **탭 색이 잘 안 보여요** → 현재 선택된 시트의 탭 색은 얇은 선으로만 표시됩니다. 다른 시트를 클릭하면 색이 확실하게 보입니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **CSV 저장 시 경고 메시지가 나와요** → 정상입니다! CSV는 현재 시트만, 서식 없이 저장됩니다. "확인"을 눌러 진행하세요
> - **PDF로 저장했는데 빈 페이지가 나와요** → 데이터가 있는 시트가 선택된 상태에서 저장해야 합니다
> - **CSV 파일을 다시 열었더니 서식이 다 사라졌어요** → 정상입니다! CSV는 데이터만 저장하는 형식이므로, 원본은 반드시 `.xlsx`로 별도 보관하세요
...
> ⚠️ **막힐 수 있는 포인트**:
> - **시트 복사가 안 되고 이동만 돼요** → `Ctrl` 키를 **누른 채** 드래그해야 복사됩니다. `Ctrl` 없이 드래그하면 이동입니다
> - **복사된 시트 이름 뒤에 (2)가 붙어요** → 정상입니다! 같은 통합 문서 안에서 시트 이름은 중복될 수 없으므로 자동으로 번호가 붙습니다
