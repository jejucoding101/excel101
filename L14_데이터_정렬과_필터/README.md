...
> ⚠️ **막힐 수 있는 포인트**:
> - **정렬했더니 데이터가 뒤섞였어요** → 정렬 전에 반드시 **표 전체**가 포함되도록 선택하세요. 한 열만 선택하면 해당 열만 재배치되어 데이터가 엉킬 수 있습니다!
> - **필터 화살표(▼)가 안 보여요** → `Ctrl+Shift+L`을 눌러 자동 필터를 켜세요. 또는 데이터 탭 → "필터" 버튼을 클릭하세요
> - **필터를 적용했는데 행 번호가 파란색이에요** → 정상입니다! 필터가 적용된 상태에서는 보이는 행의 번호가 파란색으로 표시됩니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **"현재 선택 영역으로 정렬"을 실수로 눌렀어요** → 즉시 `Ctrl+Z`로 되돌리세요! 이 실습의 목적이 바로 이 실수를 체험하는 것입니다
> - **정렬 경고 대화상자가 안 나타나요** → 표 전체를 선택한 상태에서 정렬하면 경고 없이 전체가 안전하게 정렬됩니다. 일부만 선택해야 경고가 나타납니다
...
> ⚠️ **막힐 수 있는 포인트**:
> - **Ctrl+T가 안 돼요** → 데이터가 있는 셀 범위 안에 커서를 놓은 상태에서 눌러야 합니다. 빈 영역에서 누르면 범위를 직접 지정해야 합니다
> - **"내 표에 머리글이 있습니다" 체크박스가 뭔가요?** → 1행이 제목 행이면 체크하세요. 체크하지 않으면 엑셀이 자동으로 "열1, 열2..."라는 머리글을 추가합니다
